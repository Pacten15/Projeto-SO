...
pthread_mutex_t dir_entry_lock;
    ...
    for (int inumber = 0; inumber < INODE_TABLE_SIZE; inumber++) {
        ...
        if (freeinode_ts[inumber] == FREE) {
            ...
            if (n_type == T_DIRECTORY) {
                ...
                for(int i=0;i<MAX_NUM_BLOCKS+1;i++)
...
int free_all_blocks(int inumber){
    if (inode_table[inumber].i_size > 0) {
        pthread_mutex_lock(&inode_table_lock);
        for (int i=0; data_block_get(inode_table[inumber].i_data_block[i]) != NULL && i < MAX_NUM_BLOCKS; i++){
        ...
        int*block = data_block_get(inode_table[inumber].i_data_block[MAX_NUM_BLOCKS]);
        if (block != NULL){
            for (size_t i=0; i < (inode_table[inumber].index_block_offset)/sizeof(int); i += sizeof(int)){
                if ((data_block_free(block[i])) == -1) {
                    pthread_mutex_unlock(&inode_table_lock);
                    return -1;
                }
            }
            if (data_block_free(inode_table[inumber].i_data_block[MAX_NUM_BLOCKS]) == -1) {
                pthread_mutex_unlock(&inode_table_lock);
                return -1;
            }
            inode_table[inumber].index_block_offset = 0;
        }
        pthread_mutex_unlock(&inode_table_lock);
